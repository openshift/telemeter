...
RUN cd /go/src/github.com/openshift/telemeter && go build ./cmd/telemeter-client && go build ./cmd/telemeter-server && go build ./cmd/authorization-server
...
COPY --from=0 /go/src/github.com/openshift/telemeter/telemeter-server /usr/bin/authorization-server