...
$(EMBEDMD): .bingo/embedmd.mod
	...
	@cd .bingo && $(GO) build -modfile=embedmd.mod -mod=readonly -o=$(GOBIN)/embedmd-v1.0.0 "github.com/campoy/embedmd"
...
$(GOJSONTOYAML): .bingo/gojsontoyaml.mod
	...
	@cd .bingo && $(GO) build -modfile=gojsontoyaml.mod -mod=readonly -o=$(GOBIN)/gojsontoyaml-v0.0.0-20200602132005-3697ded27e8c "github.com/brancz/gojsontoyaml"
...
$(GOLANGCI_LINT): .bingo/golangci-lint.mod
	...
	@cd .bingo && $(GO) build -modfile=golangci-lint.mod -mod=readonly -o=$(GOBIN)/golangci-lint-v1.18.0 "github.com/golangci/golangci-lint/cmd/golangci-lint"

JB := $(GOBIN)/jb-v0.2.0
$(JB): .bingo/jb.mod
	@# Install binary/ries using Go 1.14+ build command. This is using bwplotka/bingo-controlled, separate go module with pinned dependencies.
	@echo "(re)installing $(GOBIN)/jb-v0.2.0"
	@cd .bingo && $(GO) build -modfile=jb.mod -mod=readonly -o=$(GOBIN)/jb-v0.2.0 "github.com/jsonnet-bundler/jsonnet-bundler/cmd/jb"

JSONNET := $(GOBIN)/jsonnet-v0.16.0
$(JSONNET): .bingo/jsonnet.mod
	@# Install binary/ries using Go 1.14+ build command. This is using bwplotka/bingo-controlled, separate go module with pinned dependencies.
	@echo "(re)installing $(GOBIN)/jsonnet-v0.16.0"
	@cd .bingo && $(GO) build -modfile=jsonnet.mod -mod=readonly -o=$(GOBIN)/jsonnet-v0.16.0 "github.com/google/go-jsonnet/cmd/jsonnet"

JSONNETFMT := $(GOBIN)/jsonnetfmt-v0.16.0
$(JSONNETFMT): .bingo/jsonnetfmt.mod
	@# Install binary/ries using Go 1.14+ build command. This is using bwplotka/bingo-controlled, separate go module with pinned dependencies.
	@echo "(re)installing $(GOBIN)/jsonnetfmt-v0.16.0"
	@cd .bingo && $(GO) build -modfile=jsonnetfmt.mod -mod=readonly -o=$(GOBIN)/jsonnetfmt-v0.16.0 "github.com/google/go-jsonnet/cmd/jsonnetfmt"

UP := $(GOBIN)/up-v0.0.0-20200603110215-8a20b4e48ac0
$(UP): .bingo/up.mod
	@# Install binary/ries using Go 1.14+ build command. This is using bwplotka/bingo-controlled, separate go module with pinned dependencies.
	@echo "(re)installing $(GOBIN)/up-v0.0.0-20200603110215-8a20b4e48ac0"
	@cd .bingo && $(GO) build -modfile=up.mod -mod=readonly -o=$(GOBIN)/up-v0.0.0-20200603110215-8a20b4e48ac0 "github.com/observatorium/up/cmd/up"
