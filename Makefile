...
test-integration-v1: build | $(THANOS_BIN) $(UP) $(MEMCACHED_BIN) $(PROMETHEUS_BIN)
...
$(PROMETHEUS_BIN): $(BIN_DIR)
	...
	curl -L "https://github.com/prometheus/prometheus/releases/download/v2.41.0/prometheus-2.41.0.$$(go env GOOS)-$$(go env GOARCH).tar.gz" | tar --strip-components=1 -xzf - -C $(BIN_DIR)

$(THANOS_BIN): $(BIN_DIR)
	@echo "Downloading Thanos"
	curl -L "https://github.com/thanos-io/thanos/releases/download/v0.29.0/thanos-0.29.0.$$(go env GOOS)-$$(go env GOARCH).tar.gz" | tar --strip-components=1 -xzf - -C $(BIN_DIR)